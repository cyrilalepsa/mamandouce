...
APP_BANNERS_FOLDER = "mamandouce/app-banners"
...
def _asset_public_id(asset_key: str) -> str:
    slug = asset_key.replace(".", "-").replace("_", "-").lower()
    return slug[:120] or "app-banner"


def upload_app_banner(
    content: bytes,
    *,
    filename: str,
    content_type: str,
    ...
    api_secret = config.CLOUDINARY_API_SECRET
    folder = (
        os.environ.get("CLOUDINARY_APP_BANNERS_FOLDER") or APP_BANNERS_FOLDER
    ).strip().rstrip("/")
    if not cloud or not api_key or not api_secret:
        raise RuntimeError("Cloudinary n'est pas configuré")

    timestamp = int(time.time())
    public_id = _asset_public_id(asset_key)
    sign_params = {
        "folder": folder,
        "invalidate": "true",
        "overwrite": "true",
