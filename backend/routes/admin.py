...
@router.get("/admin/kpi-stats")
async def get_kpi_stats(admin: User = Depends(get_admin_user)):
    ...
    # Premium users (real paid premiums + promo codes)
    premium_users = len([u for u in users if u.get("subscription_status") == "premium"])
    
    # Conversion rate
    conversion_rate = round((premium_users / total_users * 100), 1) if total_users > 0 else 0
    
    # Gold godmothers (Marraines Or) - users with gold_status = True
    gold_godmothers = len([u for u in users if u.get("gold_status")])
    
    # Alternative: Calculate based on criteria (3 referrals + 5 contributions)
    if gold_godmothers == 0:
        # Count users who meet the gold criteria
        for user in users:
            user_id = user.get("id")
            if user_id:
                ...
                if progress:
                    referrals = progress.get("referrals_completed", 0)
                    contributions = progress.get("contributions_validated", 0)
                    if referrals >= 3 and contributions >= 5:
                        gold_godmothers += 1
    ...
    return {
        ...
        "gold_godmothers": gold_godmothers
    }


@router.get("/admin/city-stats")
async def get_city_stats(admin: User = Depends(get_admin_user)):
    """Get statistics grouped by city for the map"""
    from collections import defaultdict
    
    # Get all users with city field
    all_users = await db.users.find({}, {"_id": 0, "email": 1, "city": 1}).to_list(10000)
    users = [u for u in all_users if not is_test_user(u.get("email", ""))]
    ...
    for user in users:
        ...
        if city and city.strip():
            ...
            city_counts[city_normalized] += 1
        else:
            city_counts["Non renseignée"] += 1
    ...
    return {
        "cities": cities,
        "total_cities": len([c for c in cities if c["city"] != "Non renseignée"]),
        "total_users_with_city": sum(c["count"] for c in cities if c["city"] != "Non renseignée")
    }
...
async def update_food_status(food_id: str, status: str, admin: User = Depends(get_admin_user)):
    ...
    # Récupérer l'aliment pour connaître l'utilisateur qui l'a soumis
...
async def delete_all_messages(admin: User = Depends(get_admin_user)):
    """Delete all messages (clear inbox)"""
    result = await db.admin_messages.delete_many({})
    
    return {
        "success": True, 
        "message": f"{result.deleted_count} message(s) supprimé(s)",
...
async def send_business_kit_email(admin: User = Depends(get_admin_user)):
    ...
    <body>
        ...
        <ul>
            <li><a href="https://tirelire-staging.preview.emergentagent.com/docs/BUSINESS_PLAN_MAMANDOUCE.md">Plan Business (Markdown)</a></li>
            <li><a href="https://tirelire-staging.preview.emergentagent.com/docs/CARTE_VISITE_MAMANDOUCE.html">Carte de Visite (HTML)</a></li>
        </ul>
