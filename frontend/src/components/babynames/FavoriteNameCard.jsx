...
import { User, Trash2, ChevronDown, Share2 } from 'lucide-react';

export default function FavoriteNameCard({ 
  nameData, 
  gender, 
  country, 
  onRemove,
    ...
    <div className={`${bgColor} ${borderColor} border rounded-xl overflow-hidden`}>
      ...
      >
        ...
        <button
          ...
          className={`p-1 ${isDarkMode ? 'hover:bg-red-900/50' : 'hover:bg-red-100'} rounded-full transition-colors`}
          data-testid={`remove-favorite-${nameData.name}`}
        >
        ...
        <div className="px-3 pb-3 space-y-2">
          ...
          {onShare && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                onShare(nameData, gender, country);
              }}
