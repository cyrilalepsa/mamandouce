    ...
    <div className="min-h-screen gradient-bg flex items-center justify-center p-6">
      <Card className={`w-full max-w-md rounded-3xl p-8 shadow-[0_20px_50px_rgb(0,0,0,0.08)] border text-center animate-fade-in ${cardBg}`}>
        <div className="relative w-20 h-20 mx-auto mb-6">
          ...
          <div className={`absolute inset-2 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
            <Loader className="w-10 h-10 text-sky-500 animate-spin" />
          </div>
        </div>
        
        <h2 className={`text-2xl font-bold mb-4 ${textPrimary}`} style={{ fontFamily: 'Nunito, sans-serif' }}>
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                  isActive ? 'bg-gradient-to-r from-sky-400 to-pink-400' : isDarkMode ? 'bg-slate-600' : 'bg-slate-200'
                }`}>
                  {index < step ? (
                    <Check className="w-4 h-4 text-white" />
                  ) : (
                    <StepIcon className={`w-4 h-4 ${isActive ? 'text-white' : isDarkMode ? 'text-slate-400' : 'text-slate-400'}`} />
                  )}
                </div>
                <span className={`text-sm font-medium ${isActive ? textPrimary : textMuted}`}>
                  {s.label}
                </span>
              </div>
            );
